...
import com.qiaoyn.publish.PublishConsumer;
 ...
 */
public class RoutingConsumer implements Runnable{

    public static void main(String[] args) {
        // 定义2个线程，线程名称就用队列名称（投机取巧，避免写2个消费者实例，代码一样只是绑定的队列要不同）
        new Thread(new RoutingConsumer(),"my-queue1").start();
        new Thread(new RoutingConsumer(),"my-queue2").start();
    }
    ...
    public void run() {
        ...
        try {
             ...
             */
            channel.queueBind(name, "routing-exchange", "");
